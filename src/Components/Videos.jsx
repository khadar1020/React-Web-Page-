...
import { React, useState } from 'react'
  ...
  return (
    <Stack direction={['column', 'row']} h={'100vh'}>
      <VStack w={'full'}>
        <video controls src={videosrc} controlsList='nodownload' style={{
          width: '100%'
        }}></video>

        <VStack alignItems={'flex-start'} p={'8'} w={'full'} overflowY={'auto'}>
          <Heading>Arrays</Heading>
          <Text>This is a video on Array Basics it is use full to understand the core concepts of the Arrays</Text>
        </VStack>
      </VStack>
      <VStack w={['full', 'xl']} alignItems={'stretch'} p={'8'} spacing={'8'} overflowY={'auto'}>
        {
          ...
          <Button variant={'ghost'} colorScheme='purple' onClick={()=>setvideosrc(item)}>
            Introduction to Arrays {index+1}
        </Button>
          ))
        }
      </VStack>
